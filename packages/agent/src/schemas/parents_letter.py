...
class ParentSupport(BaseModel):
    ...
    living_support: Optional[str] = Field(
        default=None, description="生活费支持金额（可选）。如：AUD 1,500 per month"
    )

    funds_source: Optional[str] = Field(
        default=None, description="资金来源（可选）。如：stable income, savings, assets"
    )

    cover_bond: bool = Field(default=True, description="是否明确表示可承担押金（bond）")
    ...
    prepay_option: bool = Field(
        default=False, description="是否表明可预付数月租金以增强可靠性（可选）"
    )
...
class ParentLetterInput(BaseModel):
    ...
    parent_relationship: str = Field(
        description="与申请人关系，如 father / mother / guardian"
    )
    ...
    parent_location: Optional[str] = Field(
        default=None, description="所在地（可选，如 Hangzhou, China）"
    )
    ...
    statement_notes: Optional[str] = Field(
        default=None, description="附加说明（可选），如可补充提供文件、愿意电话沟通等"
    )

    letter_date: Optional[str] = Field(
        default=None, description="落款日期（可选）。不填将自动使用今天日期"
    )
    signature_name: Optional[str] = Field(
        default=None, description="签名显示名（可选）。不填默认使用 parent_full_name"
    )
