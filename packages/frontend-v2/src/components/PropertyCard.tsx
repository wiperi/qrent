...
export default function PropertyCard({ address, region, price, bedroomCount, bathroomCount, propertyType, descriptionEn, commuteTime, url }: PropertyCardProps) {
  ...
  const content = (
    <>
      ...
      <div className="mt-2 flex items-center gap-4 text-sm text-slate-600">
        {bedroomCount && <span>{bedroomCount} bed{bedroomCount !== 1 ? 's' : ''}</span>}
        {bathroomCount && <span>{bathroomCount} bath{bathroomCount !== 1 ? 's' : ''}</span>}
      ...
      {descriptionEn && (
        <p className="mt-2 text-sm text-slate-600 line-clamp-2">{descriptionEn}</p>
      )}
    </>
  )
