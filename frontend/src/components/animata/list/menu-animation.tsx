    ...
    <div className="flex min-w-fit flex-col gap-2 overflow-hidden px-10">
      {menuItems.map((item, index) => (
        <div key={index} className="group flex items-center gap-2">
          ...
          <h1 className="z-10 -translate-x-6 cursor-pointer font-mono font-semibold text-morandi-green transition-transform duration-300 ease-out group-hover:translate-x-0 group-hover:text-morandi-blue dark:text-white md:-translate-x-12 md:text-5xl md:group-hover:translate-x-0">
            {item}
          </h1>
        </div>
      ))}
